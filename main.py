...
import numpy as np
import time
from tt import recommend_similar_movies, load_movies_csv, preprocess_genres
...
def select_movie():
    ...
    selected_movie_index = movie_index[0]

    # Genereer aanbevolen films op basis van de gelijkenis met de geselecteerde film
    recommended_movies = recommend_similar_movies(df_genres, selected_movie_index)

    # Update de lijst met aanbevolen films
    listbox_recommendations.delete(0, tk.END)
    for movie in recommended_movies:
        listbox_recommendations.insert(tk.END, movie)


# Maak een tkinter-applicatie
window = tk.Tk()
window.title("Movie Recommender")
window.geometry("400x300")

# Laad het csv-bestand met films
df_movies = load_movies_csv("movie.csv")

# Voorverwerking van de genres in het gegevensframe
df_genres = preprocess_genres(df_movies)

# GUI-elementen
label_title = ttk.Label(window, text="Welkom bij de  Movie Recommender", font=("Helvetica", 16), foreground="#e50914")
label_title.pack(pady=20)

frame_input = ttk.Frame(window)
frame_input.pack()

label_input = ttk.Label(frame_input, text="Voer een film in:", font=("Helvetica", 12), foreground="red")
label_input.grid(row=0, column=0, padx=10, pady=10)

movie_entry = ttk.Entry(frame_input, font=("Helvetica", 12))
movie_entry.grid(row=0, column=1, padx=10, pady=10)

button_select = ttk.Button(frame_input, text="Zoeken", command=select_movie)
button_select.grid(row=0, column=2, padx=10, pady=10)

label_recommendations = ttk.Label(window, text="Aanbevelingen:", font=("Helvetica", 14, "bold"), foreground="#e50914")
label_recommendations.pack(pady=20)

frame_recommendations = ttk.Frame(window)
frame_recommendations.pack()

listbox_recommendations = tk.Listbox(frame_recommendations, font=("Helvetica", 12), selectbackground="#e50914", selectforeground="white", bg="#2a2a2a", fg="white")
listbox_recommendations.pack(padx=10, pady=10)
